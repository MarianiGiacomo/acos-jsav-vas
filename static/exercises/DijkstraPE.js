...
(function ($) {
  ...
  function dijkstra(nodes, distances, av) {
    ...
    while (true) {
      ...
      if (min === Infinity || nodeIndex === -1) {
        av.umsg("The nearest unvisited node is at infinite distance. The algorithm ends.");
        av.step();
        break;
      }
      ...
      if (nodeIndex === 0) {
        ...
        av.umsg("Select {node}, since it has the shortest distance from A.", {fill: {node: node.value()}});
      }
      av.step();
      ...
      if (prev) {
        av.umsg("Add edge ({from}, {to}) to the path tree.", {fill: {from: prev.value(), to: node.value()}});
      ...
      av.umsg("Update {node}'s neighbors distances. For updated nodes, set the parent node to {node}", {fill: {node: node.value()}});
      av.step();
  ...
  // This workd with jquery 2.1.4 but not older or later versions
  ...
  // Instead of jquery use document api to catch edge events
